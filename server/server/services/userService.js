...
export async function loginUser(email, password) {
    ...
    // Include the email in the JWT payload so that `/users/me` can
    // properly return the authenticated user's email.  Without
    // including the email here, `decoded.email` would be undefined and
    // `/users/me` would respond with `email: undefined` even though
    // the user has a valid email stored in the database.  See
    // userController.js for the `/me` route implementation.
